...
//
// 第四轮（#699 诊断构建，2026-07-17；针对"菜单事务"假说）：#699 报告者
// 102s 录屏帧级分析实锤——同一 showInactive()：second-instance 干净时刻
// 执行 2/2 救回病态透明，托盘菜单 click 回调内执行（"把宠物拉回主屏"）
// 0/2 失败。头号嫌疑：托盘原生菜单 click/关闭事务与 show 的交互。本轮新增：
//   • getMenuState：每张快照带 menu[...] —— 托盘/右键菜单开闭状态 +
//     最近一次被点击的菜单项 label 与相对时刻。视频考古最缺的就是
//     "点了哪一项"的 ground truth，靠光标轨迹猜会翻车（已翻过）。
//   • wrap recoverIfCloaked（#701 自愈）：结果【变化】时记一条（watchdog
//     每 5s 调用，恒定结果不刷屏）；recovered/failed 每次必记。
//   • displays 行带 colorSpace/colorDepth —— HDR（Advanced Color）是
//     #699 的头号环境变量，快照直接留底。
//   • 暴露 snapshotNow(reason)/note(line)：托盘诊断按钮（记录状态 /
//     立即重新展示 / 3 秒后重新展示）与 second-instance 打点从外部触发。
...
function createCloakDiagnostic(options = {}) {
  ...
  const getMenuState = typeof options.getMenuState === "function" ? options.getMenuState : null;
  ...
  function displaySnapshot() {
    ...
    try {
      const ds = screen.getAllDisplays().map((d) => {
        const b = d.bounds || {};
        const w = d.workArea || {};
        // 第四轮：colorSpace/colorDepth 直接留底——HDR（Advanced Color）
        // 开启时 colorSpace 会呈现 HDR/scRGB 特征，是 #699 的头号环境变量。
        return `#${d.id}{b:${b.x},${b.y} ${b.width}x${b.height} wa:${w.x},${w.y} ${w.width}x${w.height} sf:${d.scaleFactor} depth:${d.colorDepth}/${d.depthPerComponent} cs:${JSON.stringify(d.colorSpace || "?")}}`;
      });
  ...
  function contextLine() {
    ...
    // 第四轮：菜单开闭状态 + 最近点击的菜单项——"show 是否发生在菜单
    // click/关闭事务里"正是本轮要钉死的变量。
    if (getMenuState) {
      try { parts.push(String(getMenuState())); }
      catch (err) { parts.push(`menu:err ${err && err.message}`); }
    }
    return parts.join(" | ");
  }
  ...
  function instrumentPetRuntime() {
    ...
    // recoverIfCloaked（#701 自愈）单独包：watchdog 每 5s 调用，恒定的
    // clean/unavailable 不刷屏——结果【变化】记一条；recovered/failed
    // 是自愈实际出手的证据，每次必记并补完整快照。
    const recoverOrig = petRuntime.recoverIfCloaked;
    if (typeof recoverOrig === "function") {
      runtimePatches.push(["recoverIfCloaked", recoverOrig]);
      let lastRecoverResult = null;
      petRuntime.recoverIfCloaked = function diagWrappedRecover(...args) {
        let result;
        try {
          result = recoverOrig.apply(this, args);
          return result;
        } finally {
          const noisy = result === "recovered" || result === "failed";
          if (noisy || result !== lastRecoverResult) {
            log(`>>> recoverIfCloaked() -> ${shortValue(result)} (prev=${shortValue(lastRecoverResult)}) caller: ${callerFrames()}`);
            if (noisy) {
              try { dumpFull(`recoverIfCloaked:${result}`); } catch {}
            }
          }
          lastRecoverResult = result;
        }
      };
    }
  ...
  return {
    ...
    // 第四轮：外部触发接口。snapshotNow 给托盘诊断按钮/second-instance
    // 打点用（完整快照 + 强制 DOM/像素采样）；note 记单行事件标记。
    snapshotNow(reason) {
      try {
        dumpFull(reason || "manual");
        capturePetPixels(reason || "manual", true);
        domReport(reason || "manual", true);
      } catch (err) {
        log(`snapshotNow error: ${err && err.message}`);
      }
    },
    note(line) {
      try { log(`### ${line}`); } catch {}
