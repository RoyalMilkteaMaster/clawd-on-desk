...
> 支援 Windows 11、macOS 和 Ubuntu/Linux。Windows 發布版本提供獨立的 x64 和 ARM64 安裝檔。從原始碼執行需要 Node.js。支援 **Claude Code**、**Codex CLI**、**Copilot CLI**、**Gemini CLI**、**Antigravity CLI (agy)**、**Cursor Agent**、**CodeBuddy**、**Kiro CLI**、**Kimi Code CLI（Kimi-CLI）**、**Qwen Code**、**opencode**、**MiMo Code**、**Pi**、**OpenClaw**、**Hermes Agent** 與 **Qoder**。
...
- **MiMo Code** — [外掛整合](https://opencode.ai/docs/plugins)，寫入 `~/.config/mimocode/mimocode.jsonc`（Clawd 啟動時自動註冊，或執行 `node hooks/mimocode-install.js`）；與 opencode 同源，共享 `@mimo-ai/plugin` SDK，因此具備零延遲事件流、Allow/Always/Deny 權限對話框、`task` 工具分派平行子代理時自動播放建築動畫
...
一般使用者建議直接從 **[GitHub Releases](https://github.com/RoyalMilkteaMaster/clawd-on-desk/releases/latest)** 下載最新的預先建置安裝檔：
...
Discord Bot 與 LINE Messaging API 手機通知為選配功能。需要時請依照 **[Discord／LINE 選配通知安裝指南](docs/guides/notifications.zh-TW.md)** 設定；一般使用者不需要編輯 JSON。
...
**Claude Code**、**Codex CLI**、**Copilot CLI** 會自動註冊 hooks，開箱即用。**Gemini CLI**、**Antigravity CLI (agy)**、**Cursor Agent**、**CodeBuddy**、**Kiro CLI**、**Kimi Code CLI（Kimi-CLI）**、**Qwen Code**、**opencode**、**MiMo Code**、**Pi**、**OpenClaw**、**Hermes Agent**、**Qoder** 在已安裝的前提下，會在 Clawd 啟動時自動同步（OpenClaw 還需要已有設定）。也涵蓋遠端 SSH、WSL 及平台說明（macOS 與 Linux）：**[設定指南（簡體中文）](docs/guides/setup-guide.zh-CN.md)**
...
Clawd on Desk 是社群驅動的專案。歡迎提 Bug、提需求、提 PR —— 在 [Issues](https://github.com/RoyalMilkteaMaster/clawd-on-desk/issues) 聊聊或直接送 PR。
